...
use crate::RealType;
use ndarray::{Array1, ArrayView1, ArrayView2, ArrayViewMut1, Axis};
use num;
    ...
    Zip::from(result.view_mut())
        ...
        .for_each(|result_value, diff_value| {
            *result_value = num::Complex::new(
                exp_factor * m_inv_4pi * (wavenumber.re * *diff_value).cos()
                    / *diff_value,
                exp_factor * m_inv_4pi * (wavenumber.re * *diff_value).sin()
                    / *diff_value,
            );
        });
    Zip::from(result.view_mut())
