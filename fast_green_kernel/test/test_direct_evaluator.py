...
def test_laplace_evaluate_only_values(dtype, rtol, parallel):
    ...
    actual = evaluate_laplace_kernel(targets, sources, charges, dtype=dtype, parallel=parallel)
...
def test_laplace_evaluate_values_and_deriv(dtype, rtol, parallel):
    ...
    actual = evaluate_laplace_kernel(targets, sources, charges, dtype=dtype, return_gradients=True, parallel=parallel)
...
def test_helmholtz_assemble(dtype, rtol, parallel):
    ...
    actual = assemble_helmholtz_kernel(targets, sources, wavenumber, dtype=dtype, parallel=parallel)
    ...
    for index, target in enumerate(targets.T):
        ...
        expected[index, :] = np.exp(1j * wavenumber * dist) / (
            4 * np.pi * dist
        )
